...
fn test_star_text_and_feed() {
    let encoder = Encoder::star();
    let bytes = encoder
        .encode(&[Command::Text("Star Line\n".to_string()), Command::Feed(2)])
        .unwrap();
...
fn test_star_line_spacing_and_modes() {
    ...
    assert_eq!(
        encoder.encode(&[Command::LineSpacing(Some(24))]).unwrap(),
        vec![0x1B, b'3', 24]
    );
    assert_eq!(
    ...
    assert_eq!(
        encoder
            .encode(&[Command::CodePage(CodePage::Pc850)])
            .unwrap(),
        vec![0x1B, 0x1D, b't', 0]
    );
}
...
fn test_star_qr_code() {
    ...
    assert!(
        bytes
            .windows(6)
            .any(|w| w == [0x1B, 0x1D, b'y', b'S', b'0', 2])
    );
    // 2. Correction: ESC GS y S 1 1 (M = 1)
    assert!(
        bytes
            .windows(6)
            .any(|w| w == [0x1B, 0x1D, b'y', b'S', b'1', 1])
    );
    // 3. Cell size: ESC GS y S 2 4
    assert!(
        bytes
            .windows(6)
            .any(|w| w == [0x1B, 0x1D, b'y', b'S', b'2', 4])
    );
    // 4. Print: ESC GS y P
...
async fn test_star_printer_mock_and_receipt_reuse() {
    ...
    printer
        .print(&receipt)
        .await
        .expect("Star print should succeed");
