...
4. **Universal Ecosystem**: Use `papermint` in pure Rust, in Node.js/TypeScript via N-API (`papermint`), over HTTP via `papermintd`, in React Native/Expo (`expo-papermint`), in Flutter (`dart:ffi`), or in native iOS (Swift), Android (Kotlin), and Kotlin Multiplatform (KMP).
